...
5. Created `/etc/frat_secrets.json` with placeholders for `telegram_token`, `telegram_chat_id`, and `rclone_remote`, owned by `root:fratuser` with `600` permissions.
...
- Populate `/etc/frat_secrets.json` with production values for `telegram_token`, `telegram_chat_id`, and `rclone_remote` and re-apply `chmod 600 && chown root:fratuser /etc/frat_secrets.json`.
